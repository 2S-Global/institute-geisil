...
// const ITSkillModal = ({ isOpen, onClose, initialData, apiurl, setReload, setSuccess, setError }) => {
...
//   const [isSaving, setIsSaving] = useState(false); // Managed locally for button loading status
//   const [showDropdown, setShowDropdown] = useState(false);
//   const dropdownRef = useRef(null);

//   // 1️⃣ Fetch entire list once when the modal opens (Token-free global configuration)
//   useEffect(() => {
...
//       // If initialData has an _id, execute the EDIT (PUT) request path
//       if (initialData?._id) {
...
//         );
//       } else {
//         // Otherwise, execute the ADD (POST) request path
...
//         if (setSuccess) setSuccess(response.data.message || "Saved successfully!");
...
//       if (setError) {
//         setError(error.response?.data?.message || "An error occurred while saving details.");
//       }
//     } finally {
//       setIsSaving(false);
//     }
      ...
      <DialogContent className="sm:max-w-[540px] p-0 border-none rounded-xl gap-0 overflow-visible bg-white shadow-2xl [&>button]:hidden">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col overflow-visible">
          ...
          <div className="flex items-center justify-end px-6 py-4 border-t border-slate-100 bg-white relative z-10 gap-3">
            <Button type="button" onClick={onClose} variant="secondary" disabled={isSaving || isDeleting}>
              Cancel
            </Button>
