...
// export const EmploymentModal = ({ isOpen, onClose, onRefresh, jobId = "", editData = null }) => {
...
//       try {
//         const response = await API.get("https://api.geisil.com/api/candidate/employment/get_notice_period");
...
//       setJoiningYear(editData.joining_year ? String(editData.joining_year) : "");
//       setJoiningMonth(editData.joining_month ? String(editData.joining_month) : "");
//       setLeavingYear(editData.leaving_year ? String(editData.leaving_year) : "");
//       setLeavingMonth(editData.leaving_month ? String(editData.leaving_month) : "");
...
//           `/api/candidate/employment/random_company?company_name=${encodeURIComponent(companyName)}`,
//         );
//         if (response.data.success) {
...
//         description: "Please enter a Job Title first so the AI knows what to write about!",
//       });
//       return;
//     }
...
//     // Experience calculation logic matching the backend values
//     let computedExpYr = "";
//     let computedExpMnth = "";
...
//       let endMonth = new Date().getMonth() + 1; // 1-indexed conversion
...
//     // 100% exactly matching format for your backend
//     const payload = {
//       _id: jobId || "",
//       company_name: companyName,
...
//       leaving_month: currentlyWorking ? "" : (leavingMonth ? String(leavingMonth) : ""),
//       leaving_year: currentlyWorking ? "" : (leavingYear ? String(leavingYear) : ""),
...
//         toast({
//           title: "Success",
//           description: response.data.message || "Employment updated successfully!",
//         });
...
//         toast({
//           title: "Deleted",
//           description: response.data.message || "Employment entry removed successfully!",
//         });
//         if (onRefresh) onRefresh();
//         onClose();
//       } else {
...
//         description: error.response?.data?.message || "Error processing delete command.",
//       });
//     }
//   };
...
//       <Card className="w-full max-w-lg bg-white rounded-xl shadow-xl overflow-hidden font-sans text-gray-800 my-auto">
...
//                 type="button"
//                 onClick={() => {
//                   if (window.confirm("Are you sure you want to delete these employment details?")) {
//                     handleDelete();
//                   }
//                 }}
...
//           className="p-6 space-y-5 max-h-[calc(100vh-160px)] overflow-y-auto"
//         >
...
//                 className="w-full bg-[#f4f7f9] text-sm text-gray-800 placeholder-gray-400 border border-transparent rounded-lg px-4 py-3 pr-10 focus:outline-none focus:bg-white focus:border-gray-200 transition-colors"
//               />
...
//               <ul className="absolute left-0 right-0 top-full mt-1 max-h-48 overflow-y-auto bg-white border border-gray-200 rounded-lg shadow-lg z-50 divide-y divide-gray-50">
...
//               className="w-full bg-[#f4f7f9] text-sm text-gray-800 placeholder-gray-400 border border-transparent rounded-lg px-4 py-3 focus:outline-none focus:bg-white focus:border-gray-200 transition-colors"
//             />
//           </div>
...
//                   className="w-full bg-[#f4f7f9] text-sm text-gray-700 border border-transparent rounded-lg px-4 py-3 appearance-none focus:outline-none focus:bg-white focus:border-gray-200 cursor-pointer"
//                 >
...
//                   className="w-full bg-[#f4f7f9] text-sm text-gray-700 border border-transparent rounded-lg px-4 py-3 appearance-none focus:outline-none focus:bg-white focus:border-gray-200 cursor-pointer disabled:opacity-60"
//                   disabled={loadingNoticePeriods}
//                 >
//                   <option value="">
//                     {loadingNoticePeriods ? "Fetching options..." : "Select Notice Period"}
//                   </option>
...
//                 disabled={aiLoading}
//                 className="flex items-center space-x-1 text-xs font-semibold text-blue-600 hover:text-blue-700 disabled:opacity-50"
//               >
//                 {aiLoading ? (
...
//                 )}
//                 <span>{aiLoading ? "Generating..." : "Generate Description"}</span>
//               </button>
//             </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto">
      <Card className="w-full max-w-lg bg-white rounded-md shadow-xl overflow-hidden font-sans text-gray-700 my-auto">
        {/* Header matching image_de6b5c.png */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        ...
        >
          {/* Description Block with Trash can row aligned matching image_de6b5c.png */}
          ...
          {/* Company Name Field matching container shading in image_de6b5c.png */}
          ...
          {/* Conditional Leaving Date vs Notice Period Select layouts */}
          {!currentlyWorking ? (
            <div className="space-y-1">
              <label className="block text-xs font-bold text-gray-800">
                Leaving Date <span className="text-red-500">*</span>
          ...
          <div className="space-y-1.5">
            ...
            {/* AI Help me write pill button underneath the box exactly matching image_de6b5c.png */}
            <div className="pt-2">
              <button
                type="button"
                onClick={handleAiGeneration}
          ...
          {/* Footer action buttons layout matching color palette from image_de6b5c.png */}
