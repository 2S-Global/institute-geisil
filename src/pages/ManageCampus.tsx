...
import CampusFormModal from "@/components/institute/campus/FormModal";
...
const ManageCourses = () => {
  ...
  const filtered = list?.filter((s) =>
    s.campus_name?.toLowerCase().includes(query.toLowerCase()),
  );
  ...
  const handleDelete = async (id) => {
    try {
      const response = await api.delete(`/api/campus/${id}`);
  ...
  return (
    <DashboardLayout>
      <PageHeader
        eyebrow="Workspace"
        title="Manage Campus"
        description="Manage campus information and student counts."
        actions={
          <>
            <Button
            ...
            <table className="w-full min-w-[700px] text-sm text-center">
              <thead>
                <tr className="border-b border-border/60 text-xs uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="font-medium py-3 px-4 text-center">City</th>
                ...
                {paginatedLists.map((s) => (
                  ...
                  >
                    <td className="py-4 px-4">
                      <p className="font-semibold">{s?.campus_name}</p>
                    </td>
