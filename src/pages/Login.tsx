...
export default function Login() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      toast({ title: "Welcome back", description: "Signed in to your institute workspace." });
      navigate("/");
    }, 700);
  };
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      ...
      <aside className="relative hidden lg:flex flex-col justify-between p-12 overflow-hidden bg-gradient-brand text-primary-foreground">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            <p className="font-display font-bold text-xl leading-none">GEISIL</p>
            <p className="text-xs text-primary-foreground/70 mt-1">Institute Portal</p>
          </div>
        </div>
        ...
        <div className="relative max-w-md">
          ...
          <p className="mt-5 text-primary-foreground/80 leading-relaxed">
            Manage student employability, evaluations and placements from a single, secure workspace built for institutions.
          </p>

          <ul className="mt-10 space-y-4">
            {[
              { icon: Users2, text: "Unified roster of 12,000+ students & faculty" },
              { icon: Sparkles, text: "AI-assisted evaluations & skill mapping" },
              { icon: ShieldCheck, text: "Bank-grade security and audit trails" },
            ].map(({ icon: Icon, text }) => (
        ...
        <div className="relative text-xs text-primary-foreground/60">
          © {new Date().getFullYear()} Global Employability Information Services India Limited
        </div>
      </aside>
      ...
      <section className="flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-md">
          <div className="lg:hidden mb-8 flex items-center gap-3">
            ...
            <div>
              <p className="font-display font-bold text-lg leading-none text-foreground">GEISIL</p>
              <p className="text-xs text-muted-foreground mt-0.5">Institute Portal</p>
            </div>
          </div>

          <h2 className="font-display text-3xl font-bold text-foreground tracking-tight">Sign in to your workspace</h2>
          ...
          <p className="mt-10 text-center text-xs text-muted-foreground">
            Developed and maintained by <span className="font-semibold text-foreground">2S Global Technologies Ltd</span>
          </p>
        </div>
      </section>
    </div>
